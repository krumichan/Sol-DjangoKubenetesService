...
class NodeRunService:
    ...
    def __new__(cls):
        ...
        if not hasattr(cls, '_me'):
            # instance 생성.
            cls._me = super(NodeRunService, cls).__new__(cls)
    ...
    def _node_run_cluster(self, options):
        dummy = self

        # command 추출.
        node_command = options["nodeCommand"]
        ...
        ssh_client = SshClientSample(server=server, username=username, password=password)

        # TODO: 실패 처리 추가 필요.
        stdin, stdout, stderr = ssh_client.execute(node_command)

        # TODO: 반환 결과 출력 문자열 습득 필요.
        return True
